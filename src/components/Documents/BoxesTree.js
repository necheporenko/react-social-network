...
import '../BooksTree/draggable.scss';

@connect((state) => ({
  // boxes: state.book.boxes,
...
class BoxesTree extends Component {
  constructor(props) {
    super(props);
    [
      'onDragStart',
      ...
      data.map((item) => {
          return (
            <TreeNode
              ...
              className={[[`${this.onPick(item)}`], [`${path}` === `${`/${slug}/documents/${item.key}`}` ? 'active' : null]]}
              disabled={item.no_drag}
            >
              <Link
                to={`/${slug}/documents/${item.key}`}
                draggable={false}
